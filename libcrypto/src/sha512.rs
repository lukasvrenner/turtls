...
fn pad_message(msg: &[u8]) -> Vec<u8> {
    let padding_size =
        (BLOCK_SIZE - (msg.len() + 17) % BLOCK_SIZE) % BLOCK_SIZE;
...
/// ];
/// assert_eq!(sha512::hash(message), hash);
/// ```
...
fn update_hash(
    hash: &mut [u64; HASH_SIZE / 8],
    next_block: &[u64; BLOCK_SIZE / 8],
) {
