...
pub fn server_hello(msg_buf: &mut [u8], csprng: impl FnOnce() -> [u8; 32], leg_session_id: &[u8], cipher_suite: CipherSuite, extensions: &[u8]) {
    let mut pos = 0;
    ...
    assert!(extensions.len() >= 6, "at least three extensions must be used");
